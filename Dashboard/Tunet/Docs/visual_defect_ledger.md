...
- ✅ **Lighting card hold gate** (commit `d18b99e` — closes corpus #11176 P0 from 2026-05-04). `_initTileDrag` now passes `longPressMs: 500` + `onLongPress` (more-info dispatch); matches `tunet_light_tile.js` shape.
- ✅ **Lighting card pill position** (same commit). `.l-tile.sliding .zone-val` repositioned dead-center inside tile (`top: 50%; transform: translate(-50%, -50%)`); reverts the `bec0839` regression that pushed pill above tile bounds.
- ✅ **Inbox card dark mode** (commit `1b6eb36`). `:host(.dark)` overrides for `.empty`, `.item`, `.action-btn.secondary`, `.action-btn.dismiss`.
- ✅ **Chrome drift across 11 cards** (commit `7488198`). `width: 100%` promoted into CARD_SURFACE; `transition: .3s` drift on weather/actions/speaker_grid replaced with `var(--motion-surface)`; sonos padding tokenized; inbox phone radius normalized.
- ✅ **SA5 Sonos snooze re-trigger** (commit `28069d4`). 60s discriminator on `automation.sonos_reset_snoozed_alarm_after_play`; verified live via time-resolved attribute trace.
...
- `S.1 deploy script does NOT handle new package files (2026-05-26, logged for future fix)`
  ...
  - **Future fix**: extend `deploy_packages.sh` to detect local-only files, prompt user to confirm (or auto-create with empty remote first), then deploy. OR document the manual SCP step as a required pre-deploy action for any new file.

- `light.office_bed_light_right persistent manual-control flag (2026-05-26, deferred to follow-up)`
  - **Defect**: `switch.adaptive_lighting_office_bed.manual_control` attribute carries `["light.office_bed_light_right"]` with `autoreset_time_remaining: ~14000s`. Only the RIGHT bed light is flagged; left is not. Both bed lights receive the same `automation.oal_v13_office_bed_lights_warm_pin` warm-pin augmentation per `packages/oal_lighting_control_package.yaml` — yet only the right's lock persists.
  - **Hypothesis**: warm-pin automation calls `set_manual_control: "color"` per Adaptive Lighting's per-attribute lock semantic (CLAUDE.md OAL principles), but only one of the two bed lights' locks reliably persists across AL adaptation cycles. Possible causes: (1) race between automation completion + AL re-adaptation; (2) bug in warm-pin automation iteration over the two lights; (3) AL silently maps `"color"` → `false` on one of them (the "any string other than 'color'/'brightness' silently maps to false" footgun documented in CLAUDE.md).
  - **Impact**: cosmetic in dashboard (light shows as "manually controlled"); functional state unclear — bed light brightness may not be tracking AL's CCT pin.
  - **Investigation deferred**: not S.1 scope. Targets `automation.oal_v13_office_bed_lights_warm_pin` + state probe during a warm-pin firing cycle to trace which call lands successfully.

- `T8.1-regression caught by M.1 investigation (2026-05-26, resolved)`
  - **Defect**: T8.1 wired `compactSpeakerName(spk.name || spk.entity)` into the speaker-grid render at `tunet_speaker_grid_card.js:1149` but failed to add `compactSpeakerName` to the import block at lines 17-37. Production speaker-grid threw `ReferenceError: compactSpeakerName is not defined` on every `_buildGrid()` call from commit `e35d43e` (2026-05-24 02:06am MDT) until M.1 fix (2026-05-26 ~16:03 MDT). Net: ~62 hours of broken speaker-grid render in production. The "Speakers / Loading" state I observed during T8.1 M1 production-mirror capture was the card failing to mount, NOT a transient loading state.
  - **How it shipped**: T8.1's M1 capture missed this because the speaker-grid section was below the visible viewport in screenshots I read inline, and I assumed "Loading" was transient. M1 contract says "read each capture inline" — I did, but the section showing the JS error was visually ambiguous without further investigation.
  - **How M.1 caught it**: running `npm test` against the full vitest suite as part of the M.1 implementation gate surfaced the existing `audio_cd9_bespoke.test.js` tests that mount `tunet_speaker_grid_card.js`; those tests threw the ReferenceError on every fixture, exposing the regression.
  - **Resolution**: added `compactSpeakerName` to the speaker-grid import block; bumped CARD_VERSION `'3.3.1' → '3.3.2'`.
  - **Lesson logged**: production-mirror capture as M1 evidence is necessary but not sufficient — for any card that mounts dynamically (speaker-grid is JS-rendered from `_buildGrid`), the test suite is a load-bearing second gate. Always run `npm test` after any card source change, not just when adding new tests.

- `M.1 Sonos+Spotify transport routing (2026-05-26, WITHDRAWN — premise empirically wrong)`
  - **Original premise (memory #12575, Plan F §8)**: native Sonos integration rejects `media_next_track`/`media_previous_track` when Spotify owns the queue; route transport to Music Assistant shadow entity to skip.
  - **Implementation shipped**: coordinator-rooted `resolveSpotifyMAShadow()` helper in `tunet_base.js` + wiring in `tunet_media_card._callTransport` and `tunet_sonos_card._callTransport` (cards bumped to 3.2.3 / 1.1.2).
  - **Live test result**: Mac tapped next-track → "Failed to perform action next track. Media player next track is unavailable." Mac verified that native HA media-player more-info popup's next-track on `media_player.living_room` DOES work directly under Spotify Connect playback. My helper routed AWAY from the working native entity to the MA shadow, which returns "unavailable" because `active_queue: null` in Spotify Connect mode — Spotify's external app owns the queue, not MA.
  - **Root cause of the wrong premise**: memory #12575 was a stale diagnosis. Either the HA Sonos integration improved to accept Connect-mode skip OR the original audit's conclusion was wrong from the start. The probe-derived MA routing helper was technically well-engineered (correct coordinator detection, fallback handling) but solved a problem that empirically doesn't exist on Mac's current setup.
  - **Resolution**: REVERTED `_callTransport` in both cards to direct service call. Removed helper from `tunet_base.js` and its test file (YAGNI per CLAUDE.md). Cards bumped to 3.2.4 / 1.1.3 to flush cache.
  - **Lesson logged**: **always do an empirical baseline test BEFORE writing the fix** — would the user have hit the defect by tapping the native HA control? If "yes, the native control fails too" → routing helper warranted. If "no, native works fine" → there's no defect to fix; the prior audit's conclusion was wrong. M.1's plan included a "pre-implementation probe" but the probe only verified MA shadow entity existence and Spotify detection on coordinator; it did NOT empirically test whether the native `media_next_track` call actually fails. That gap is what let an entire ~45-minute implementation cycle proceed against an empirically-wrong premise.
  - **Memory cleanup**: observation #12575 ("Sonos+Spotify Transport Fix Root Cause Confirmed: Route to MA Entity") and Plan F §8/§12.5.10 should be marked SUPERSEDED — the original confirmation was incorrect for current HA/Sonos versions on Mac's instance.

- `T8.1 popup-content-card-revisions (2026-05-24, resolved-in-tranche)`
  - **Pattern**: native HA `button` cards inside `horizontal-stack` inside a Bubble Card popup with `min-height` force on `ha-card` stretch vertically to absorb leftover space. Manifested on alarm-edit desktop popup where Close/Save buttons rendered ~3x normal height. Resolution shipped: converted Close/Save to `mushroom-template-card` which has intrinsic ~56px row height. **Pattern lesson**: prefer mushroom cards over native HA button cards as bottom-row CTAs inside Bubble popups; native button cards have no intrinsic height and will absorb force-min-height empty space
  - **Climate variant: thin .hdr overflow on phone**: thin variant's CSS modifies `.card/.temps/.slider*` but NOT `.hdr` flex layout. At 390px popup width the 3 .hdr children (hdr-tile + fan-btn + mode-btn "Heat / Cool" nowrap) overlap. Resolution shipped (A2 hotfix at `tunet_climate_card.js:601-625`): phone @media override scoped to `:host([variant="thin"])` that wraps .hdr to 2 rows (identity row + fan+mode controls row, right-aligned via margin-left: auto on fan-btn)
  - **Sonos card .player-header 4-child crush**: `.player-header` has 4 flex children (album, track-info, transport, speaker-wrap) competing for 314px popup width. Speaker-wrap holds 105px (Kitchen ▼) and transport 140px, starving track-info to 6.59px (single-character truncation). Resolution shipped (`tunet_sonos_card.js:519-545`): phone @media `flex-wrap: wrap` + explicit `order: 0/1/2/3` places album+track-info on row 1, transport+speaker-wrap on row 2 with speaker-wrap right-aligned
  - **Speaker-grid tile name verbose**: tile names were rendered as full entity friendly_name ("Living Room", "Bathroom") causing truncation in narrow tiles AND verbose desktop labels. Resolution shipped (`tunet_speaker_grid_card.js:1144`): wired `compactSpeakerName()` (already imported, sourced from `tunet_base.js:1631`) to render compact single-word forms ("Living", "Bath", "Bed"). Suite-wide, not phone-only — `aria-label` at line ~1062 still uses full entity name for screen readers
