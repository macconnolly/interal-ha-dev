...
| `TI6` | diagnostics, repairs, corruption handling, deploy/test hardening | `Deferred by policy` | intentionally postponed until functional migration path is proven; 2026-05-22 review added concrete hardening backlog but user deferred implementation |
...
| `TINBOX-DEPLOY-1` | `tinbox:deploy:integration` is not part of the basic root Tunet Claude pipeline, while frontend Tinbox card deploy and backend integration deploy can drift | `Open contract gap` | `P1` | `Deferred by policy` | `TI6 / deploy hardening` | current active tranche deferred | docs + static + HA/live evidence | define an explicit Tinbox release path: frontend card build/deploy, backend integration deploy/reload, and smoke checks must be ordered together when both surfaces changed |
| `TINBOX-DEPLOY-2` | Tinbox integration deploy helper has unsafe defaults and hardcoded remote bootstrap assumptions — `scripts/deploy_tunet_inbox.sh:52` reads `HA_SSH_PASSWORD="${HA_SSH_PASSWORD:-password}"`, so an unset env var causes the script to authenticate as `root` with the literal string `"password"` against every subsequent `sshpass` invocation (lines 62, 71, 72, 231, 235, 239). The same script then mutates remote `configuration.yaml` via a python heredoc (lines 79-228), so a successful auth with the default password gives the deploy write authority over remote HA config | `Open implementation blocker` | `P1` | `Deferred by policy` | `TI6 / deploy hardening` | current active tranche deferred | static + HA/live evidence | remove literal password fallback (guard with `if [[ -z "${HA_SSH_PASSWORD:-}" ]]; then fail "..."; fi`), avoid password argv exposure (move to SSH key auth), stop replacing operator-owned `tunet_inbox:` options such as `mobile_tap_url`, and remove Mac-specific notify-device hardcoding |
| `TINBOX-HARDEN-3` | Diagnostics redacts runtime queue fields but returns config entry `data` and `options` unredacted | `Open contract gap` | `P1` | `Deferred by policy` | `TI6` | current active tranche deferred | runtime evidence | add entry-level redaction for routing metadata and actor/device identifiers, then add a regression that fails on unredacted `entry.data` / `entry.options` |
| `TINBOX-HARDEN-4` | Mobile-action and notify-target hardening gaps can turn malformed input or missing helpers into runtime errors / invalid notify services — `models.py:341-350` `parse_raw_mobile_action` documents `tuple[str, str] \| None` and the consumer at `mobile.py:135-137` checks only `if not parsed: return None`, but after `RAW_ACTION_PREFIX` matches, lines 348-349 call `_require_safe_id(...)` which raises `ContractError` on any value failing `^[A-Za-z0-9_.:-]{1,128}$`. A payload like `TINBOX\|"; drop\|x` makes it past the prefix and crashes the function with `ContractError` rather than returning `None`. The exception then propagates through `extract_mobile_response` (mobile.py:132-153) into `_on_mobile_action` (`__init__.py:143-174`) which has no try/except wrapping `extract_mobile_response`, so the HA event listener call fails for that event | `Open implementation blocker` | `P1` | `Deferred by policy` | `TI6` | current active tranche deferred | runtime evidence | make malformed `TINBOX\|...` action strings non-fatal (wrap the `_require_safe_id` calls inside `parse_raw_mobile_action` with `try / except ContractError: return None`, or catch in the `mobile.py:135` caller) and reject missing helper entity targets instead of synthesizing invalid `notify.mobile_app_input_text.*` services |
| `TINBOX-HARDEN-5` | Queue supportability edge cases remain: `list_items.meta.total` is post-limit (`manager.py:348-358` slices `items = items[:limit]` at line 349 before `len(items)` is captured at line 354), malformed ISO timestamps can crash reconcile/sweep, terminalized item locks are retained until reload | `Validation debt` | `P2` | `Deferred by policy` | `TI6` | current active tranche deferred | runtime evidence | decide total semantics, quarantine or surface invalid timestamps, and clean `_locks` when an item terminalizes |
| `TINBOX-TEST-4` | Targeted Python tests pass but miss adversarial cases found by review, creating false confidence | `Validation debt` | `P1` | `Deferred by policy` | `TI6` | current active tranche deferred | runtime evidence | add failure-first tests for diagnostics entry redaction, malformed raw mobile actions, missing notify helpers, post-limit totals, invalid timestamps, and lock cleanup before claiming backend hardening complete |
| `TINBOX-HARDEN-6` | POST service contract mismatch — `services.py:80` marks `mobile` Optional in `POST_SCHEMA`, but `models.py:213-219` (`InboxItem.from_post_payload`) invokes `InboxMobileConfig.from_dict(mobile_payload or {}, ...)` whenever `send_mobile` is not explicitly `False`, and `models.py:154` requires `mobile.tag` via `_require_safe_id`. Any caller that omits `mobile` and leaves `send_mobile` at its default (True) passes voluptuous validation and then raises `ContractError("mobile.tag must match ...")`, which `services.py:168-169` surfaces as `ServiceValidationError` against a field the caller never supplied | `Open contract gap` | `P2` | `Deferred by policy` | `TI6` | current active tranche deferred | static + runtime evidence | tighten POST_SCHEMA so `mobile` is required whenever `send_mobile` is true or absent (preferred), OR synthesize a default tag in `from_post_payload` when `mobile_payload` is empty. Pick the schema-tightening path to keep contract enforcement at the voluptuous boundary |
| `TINBOX-HARDEN-7` | `TunetInboxManager.async_update_config` (`manager.py:210-212`) is defined to support in-place runtime config updates but is never invoked anywhere — `__init__.py:232-234`'s `async_reload_entry` calls `hass.config_entries.async_reload(entry.entry_id)`, which fully unloads and re-creates the manager. Either the method is dead code that should be deleted, or `async_reload_entry` was supposed to use it to preserve in-memory queue state (active_items, locks, archive) across options-only reloads. Either way the current state is intent-divergent | `Open contract gap` | `P3` | `Deferred by policy` | `TI6` | current active tranche deferred | static evidence | decide intent and either delete `async_update_config` (declare full reload is the design) or wire it into `async_reload_entry` for options-only updates. Record decision in `custom_components/tunet_inbox/AGENTS.md` reload semantics section |
...
- close `TINBOX-DEPLOY-1`
- close `TINBOX-DEPLOY-2`
- close `TINBOX-HARDEN-3`
- close `TINBOX-HARDEN-4`
- close `TINBOX-HARDEN-5`
- close `TINBOX-TEST-4`
- close `TINBOX-HARDEN-6`
- close `TINBOX-HARDEN-7`
...
- `P1`: deployment helpers are not safe enough to treat as invisible plumbing; they can drift frontend/backend releases, hardcode local notify devices, and clobber operator-owned bootstrap options
- `P1`: current local tests are useful smoke, but they do not cover the 2026-05-22 adversarial findings and must not be used as the only acceptance evidence for hardening work
