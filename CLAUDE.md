...
**Iterative UI work — `--share-with-user` mandate**: When iterating with the user in real time on a UI change, run `npm run tunet:review:share` (passes `--share-with-user`). The harness fires an HA push notification via `notify.tunet_inbox_all_devices` (override with `--notify-service <name>`) so Mac gets a phone alert with the capture count and the production URL to open. **The notification is the cue, not the artifact** — Mac opens the live dashboard on his phone after the alert. For iterative review, screenshot-paths-without-inline-content are explicitly NOT a substitute for either inline read-back OR the HA push notification.

(History note: an earlier draft of M1 mandated `SendUserFile(status='proactive')` for iPhone delivery — that primitive does not actually push to iPhone in WSL-on-laptop Claude Code sessions and was an unverified assumption corrected 2026-05-22.)
...
- **Adaptive Lighting custom integration requires a full integration reload to re-import YAML `lights:` changes.** A plain `homeassistant.reload_all` does NOT propagate `lights:` membership changes to existing AL config entries — that was the 2026-05-22 burn when office AL kept its old 4-light membership. The correct procedure is to manually reload the Adaptive Lighting integration (Settings → Devices & Services → Adaptive Lighting → Reload, or equivalent service call) AFTER YAML deploy. This re-imports the `lights:` list into existing entries; entries are NOT immutable. (Prior wording here claimed immutability + suggested `ha_delete_config_entry` / `ha_restart` as the only path — that was wrong; corrected 2026-05-26.)
