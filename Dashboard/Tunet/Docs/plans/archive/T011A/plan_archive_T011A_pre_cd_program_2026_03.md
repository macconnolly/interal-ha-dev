# T-011A — Pre-CD Program Profile System and Sizing Research Archive

> ARCHIVED 2026-05-04 — extracted from plan.md@HEAD~1 lines 865-2409. Read-only. To resume any decision threads, see `plan.md` Tranche Queue and `Dashboard/Tunet/Docs/visual_defect_ledger.md`.

**Period**: 2026-03-06 → 2026-03-14
**Status**: Superseded by Card Rehabilitation Reset (CD0-CD12) on 2026-04-02; profile resolver contract explicitly retired
**Scope**: Profile-system architecture (`v3.0` → `v3.1` all-em registry), container-width migration off viewport-primary sizing, v2 → v3 sandbox bootstrap with eventual v3 promotion, mobile density token rollout, and surface-by-surface layout planning order locks

## Synthesis

### What we did
- Stood up profile-system v3.1 in `tunet_base.js`: 5-family registry (`tile-grid`, `speaker-tile`, `rooms-row`, `indicator-tile`, `indicator-row`), size-indexed `PROFILE_BASE` (compact/standard/large), all-em token shape, `selectProfileSize()` vs `resolveSizeProfile()` API split, and `_setProfileVars()` writer (T-011A.7, T-011A.10)
- Migrated 7 cards onto the profile pipeline with `use_profiles` rollback flag and host `ResizeObserver` re-selection: lighting, speaker_grid, status, rooms (with `tile-grid` vs `rooms-row` layout-aware family routing), sensor, light_tile, plus base token expansion (T-011A.11, T-011A.15, T-011A.16, T-011A.19)
- Bootstrapped `Dashboard/Tunet/Cards/v3/` from v2 baseline (commit `32dde28`), then promoted v3 to production on 2026-03-14 by deploying 14 files to `/config/www/tunet/v3/` and switching all 13 Lovelace resources to `/local/tunet/v3/?v=20260314_v3a` (T-011A.10, T-011A.23)
- Container-width migration: enabled `container-type: inline-size` on host, switched mobile density gating from viewport media queries to `@container (max-width: 440px)`, replaced `window.resize` paths with `ResizeObserver` in status/lighting cards (T-011A.4)
- Resolved import-version cache bug where 7 of 13 v3 cards had stale `?v=` strings creating separate ES module scopes; aligned all imports to `tunet_base.js?v=20260309g7` (T-011A.23)
- Diagnosed and fixed light-tile drag flicker by adding `_isDragging` guard so `_render()` is skipped during active drag and runs once on `onDragEnd` (T-011A.23, KI-001 candidate fix)

### Why we did it
- v2 had viewport-coupled responsive paths and per-card sizing formulas that drifted across cards; goal was a single density lever (host `font-size`) and family-shared tokens
- Mobile readability and tap comfort had degraded; centralized `--density-mobile-*` tokens in T-009E let multiple cards adopt one source of truth instead of per-card overrides
- Sections-grid placement needed intrinsic `rows: 'auto'` sizing on `getGridOptions()` to play correctly with HA 2026.3 instead of fighting fixed row heights

### Files touched
- `Dashboard/Tunet/Cards/v2/tunet_base.js`, `tunet_lighting_card.js`, `tunet_status_card.js`, `tunet_rooms_card.js`, `tunet_light_tile.js`, `tunet_speaker_grid_card.js`, `tunet_scenes_card.js`, `tunet_actions_card.js`, `tunet_nav_card.js`, `tunet_weather_card.js`
- `Dashboard/Tunet/Cards/v3/tunet_base.js`, `tunet_lighting_card.js`, `tunet_speaker_grid_card.js`, `tunet_status_card.js`, `tunet_rooms_card.js`, `tunet_sensor_card.js`, `tunet_light_tile.js`
- `Dashboard/Tunet/Cards/v3/tests/profile_resolver.test.js`
- `Dashboard/Tunet/tunet-g2-lab-v3.yaml`, `tunet-suite-config.yaml`, `tunet-suite-storage-config.yaml`
- `Dashboard/Tunet/Mockups/design_language.md` (rewritten as v9.0 V2 contract spec, then `5.5` updated to all-em shape)
- `Dashboard/Tunet/design.md` (rewritten as V2 doc-structure index)
- `Dashboard/Tunet/Agent-Reviews/unified_tile_architecture_conclusion.md` (v3.1 canonical registry §7)

### Key decisions
- Option C (family profile consumption) selected as unification direction, fit score `8.15/10` (anchor: T-011A.3 / 2026-03-07)
- All-em token shape locked; every PROFILE_BASE token is a pre-formatted em string, no px or raw numbers (anchor: T-011A.7 / 2026-03-08)
- `rooms-row` is one family; `slim` is a layout-variant modifier, not a separate family (anchor: T-011A.3 / 2026-03-07)
- Status card deferred to `G3S` bugfix-only tranche; non-status family rollout proceeds without it (anchor: T-011A.20 / 2026-03-09)
- v3 is sole implementation authority post-2026-03-14; v2 retained at `/config/www/tunet/v2/` only as rollback (anchor: T-011A.23 / 2026-03-14)
- Room card-body tap = primary route (`navigate`); explicit room controls/orbs own toggle; `hold` is optional secondary, not global baseline (anchor: T-011A.6 / 2026-03-07)

### Entry points (for regression hunting)
- Tests: `Dashboard/Tunet/Cards/v3/tests/profile_resolver.test.js` (8/8 pass throughout series)
- Entities: `light.living_column_strip_light_matter`, `light.dining_column_strip_light_matter`, `light.column_lights`, `switch.adaptive_lighting_column_lights`, `input_boolean.oal_column_rgb_session_active` (T-011A.22 OAL column sunset late-on prepare gap, planning only)
- Selectors / DOM hooks: `:host([use-profiles])`, `:host(:not([use-profiles]))`, `--_tunet-*` token namespace, `--row-btn-size`, `--grid-row` (`max(var(--_tunet-tile-min-h, 110px), 6.25em)`), `profile-family` / `profile-size` host attributes, `.bubble-sub-button-bottom-container`
- Lab dashboard: `tunet-g2-lab-v3` registered in HA `configuration.yaml` → `dashboards/tunet-g2-lab-v3.yaml`

### Deferred work / handoffs
- OAL column sunset late-on prepare coverage planned (T-011A.22) but not implemented in `packages/oal_lighting_control_package.yaml`
- v2 server cleanup deferred pending v3 validation (T-011A.23 step 8 open)
- Status-card alignment tranche `G3S` queued, not started
- KI-001 light-tile drag fix deployed at `?v=20260314_v3b` pending DevTools validation

### Superseded by
- 2026-04-02 Consistency-Driver Reset (`Dashboard/Tunet/Docs/plans/archive/CD/plan_archive_CD0_CD3_consistency_driver_2026_04_02_to_04_03.md`) — profile resolver contract retired in favor of `tile-size` attribute + hand-tuned CSS variants per card

### Related claude-mem observations (added 2026-05-04)

- #7829 — unified_tile_architecture_conclusion.md Updated to v3.0 — 5-Family Taxonomy Locked (2026-03-07) — locks the 5 family keys that drive every PROFILE_BASE entry in this archive
- #7833 — Profile System V1: Full Implementation Spec — Two-Name CSS Var Pattern, 5 Family Keys, _setProfileVars() API (2026-03-07) — canonical spec for the API split (`selectProfileSize` vs `resolveSizeProfile`) and `--_tunet-*` token writer
- #7841 — Container-First Sizing Migration: tunet_base.js, tunet_status_card.js, tunet_lighting_card.js Migrated (2026-03-07) — confirms T-011A.4 viewport→container-query cutover landed across the three pilot files
- #7849 — Design Documentation Fully Rewritten as V2 Implementation Contract (v9.0) (2026-03-07) — governance sync for `design_language.md` rewrite recorded in T-011A.5
- #7851 — handoff.md Updated with Session Delta 0Q — Architecture v3.0 Finalized (2026-03-07) — handoff governance file synced with the v3.0 architecture lock
- #7880 — G1 Profile System Base Primitives Added to tunet_base.js (2026-03-07) — proves G1 implementation completion before v3 sandbox bootstrap
- #7918 — tranche_queue.md Fully Rewritten — Old Nav/Popup Queue Replaced with Gate-Based Profile Rollout (2026-03-08) — queue-discipline reset that reframed work around G1–G5 gates
- #7957 — tunet_speaker_grid_card: Full G2 Profile Methods Implemented (2026-03-08) — G2 pilot completion on the second profile-consumer card
- #8022 — tunet_status_card.js Excluded from Profile Migration (User Decision Mar 9) (2026-03-09) — locked decision matching T-011A.20 status deferral to G3S
- #8039 — G3 Profile Consumption System Added to Rooms Card (2026-03-09) — G3 rooms-row + tile-grid layout-aware family routing landed
- #8190 — G3.0 Nav Card Neutralization Deployed (2026-03-09) — nav `ensureGlobalOffsetsStyle()` neutralization prerequisite cleared
- #8347 — Profile Rollout Gates G1–G5 All Complete and Live-Deployed (2026-03-14) — confirms full G1–G5 closure ahead of v3 promotion
- #8346 — v3 IS the Active Implementation Authority — Supersedes v2 Memory Note (2026-03-14) — promotion authority lock matching T-011A.23
- #8257 — Tunet Cards v3 Path Established with Profile Pipeline and Sections-Safe getGridOptions (2026-03-14) — captures the `rows: 'auto'` Sections-safe `getGridOptions()` contract from G5
- #8248 — Known Issues KI-001/KI-002/KI-003 Full Detail Retrieved From Memory (2026-03-14) — origin record for the KI-001 light-tile drag flicker and KI-003 row toggle issues handed forward from this archive
