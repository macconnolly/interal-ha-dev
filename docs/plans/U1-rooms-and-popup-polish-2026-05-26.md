...
| **D5** | Chevron tap is BROKEN on live preview. Mac empirical ground truth 2026-05-26: "The Chevron is actually not working on the preview dashboard doesn't go anywhere. Clicking on the body of the row does work." **Correction to prior framing**: I earlier read `tunet_rooms_card.js:1316-1329` and assumed "code looks wired = works." Mac's live test refutes that — the handler is registered but the navigation isn't firing. Possible causes: (a) event bubbling/stopPropagation race with parent body-tap handler swallowing the chevron click, (b) `navigation_path` value is empty/undefined for some rooms (rooms[] without subview_path), (c) `e.stopPropagation()` missing on chevron handler letting body re-handle and override, (d) Alt II hash navigation routes through HA's `<a>` interception which silently no-ops in some HA versions. **Investigation needed in Phase 3.0**: live DOM probe + console trace on chevron click. THEN choose: fix chevron-to-subview (restore Alt II intent) OR retarget chevron-to-popup (mirror body-tap). Lesson logged: `feedback_empirical_baseline_before_fix.md` applies to CODE READING too — "code is wired" ≠ "code works." | Mac live test 2026-05-26 + `tunet_rooms_card.js:1316-1329` | Card-level event handler |
