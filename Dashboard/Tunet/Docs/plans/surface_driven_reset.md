...
5. `tunet_light_tile.js` retains its `editor-complete` policy; `tunet_lighting_card.js` retains its `editor-lite` policy.
6. `Dashboard/Tunet/Docs/cards_reference.md` entries complete for both cards.
...
1. Climate screenshots are non-regressive against the current baseline in **both dark and light mode**.
2. Sensor remains keyboard-reachable where interactive.
3. Weather has no fake interactive containers left.
4. `tunet_climate_card.js` retains its `editor-lite` policy; `tunet_sensor_card.js` retains its `editor-complete` policy; `tunet_weather_card.js` retains its `editor-complete` policy.
5. `Dashboard/Tunet/Docs/cards_reference.md` entries complete for all three cards.
...
4. All three cards retain their `editor-lite` policy; YAML-only keys are documented.
5. `Dashboard/Tunet/Docs/cards_reference.md` entries complete for all three cards.
...
2. `tunet_nav_card.js` retains its `editor-complete` policy.
3. No new code is added unless a real failure is reproduced.
4. `Dashboard/Tunet/Docs/cards_reference.md` entry complete.
